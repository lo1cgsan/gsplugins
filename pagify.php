...
// start of modifications by wDEsign
    ...
    if ($pageNum > 0) {
      ...
      $pagingHtml .= ' <li class="page-item previous"><a class="page-link" href="'.($link1 && $pageNum == 1 ? $link1 : str_replace('%PAGE%',$pageNum,$link)).'" title="'.i18n_r('pagify/TITLE_PREVIOUS').'">'.i18n_r('pagify/PREVIOUS').'</a></li>';
    } else {
      $pagingHtml .= '<li class="page-item first disabled"><a class="page-link" href="#">'.i18n_r('pagify/FIRST').'</a></li>';
      $pagingHtml .= ' <li class="page-item previous disabled"><a class="page-link" href="#">'.i18n_r('pagify/PREVIOUS').'</a></li>';
    }
    ...
    if ($min > 0) $pagingHtml .= ' <li class="page-item ellipsis"><span>'.i18n_r('pagify/ELLIPSIS').'</span></li>';
    for ($i=$min; $i<=$max; $i++) {
      if ($i == $pageNum) {
        $pagingHtml .= ' <li class="page-item active"><a class="page-link" href="#">'.($i+1).' <span class="sr-only">Aktywna</span></a></li>';
      } else {
        $t = i18n_r('pagify/TO_PAGE');
        $pagingHtml .= ' <li class="page-item"><a class="page-link" href="'.($link1 && $i == 0 ? $link1 : str_replace('%PAGE%',$i+1,$link)).'" title="'.(trim($t) ? $t.($i+1) : '').'">'.($i+1).'</a></li>';
      }
    }
    if ($max < $numPages-1) $pagingHtml .= ' <li class="page-item ellipsis"><span>'.i18n_r('pagify/ELLIPSIS').'</span></li>';
    if ($pageNum < $numPages-1) {
      $pagingHtml .= ' <li class="page-item next"><a class="page-link" href="'.str_replace('%PAGE%',$pageNum+2,$link).'" title="'.i18n_r('pagify/TITLE_NEXT').'">'.i18n_r('pagify/NEXT').'</a></li>';
      $pagingHtml .= ' <li class="page-item last"><a class="page-link" href="'.str_replace('%PAGE%',$numPages,$link).'" title="'.i18n_r('pagify/TITLE_LAST').'">'.i18n_r('pagify/LAST').'</a></li>';
    } else {
      $pagingHtml .= ' <li class="page-item next disabled"><a class="page-link" href="#">'.i18n_r('pagify/NEXT').'</a></li>';
      $pagingHtml .= ' <li class="page-item last disabled"><a class="page-link" href="#">'.i18n_r('pagify/LAST').'</a></li>';
    }
    $pagingHtml .= ' </ul></nav></div>';
