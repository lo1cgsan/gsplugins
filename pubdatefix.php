...
// --- dodane
	...
	if (file_exists(LUPDATE_FILE)) {
		...
		foreach($xmldata as $page) {
			if ((string)$page->url == $url || (string)$page->title == $title) { //usuwamy wpis strony o ile już istnieje
				unset($xmldata->page[$k]);
				break;
			}
			$k++;
		}
		$ile=count($xmldata->children());
		if ($ile > 20) { //zostawiamy tylko 19 pozycji
			for($k=$ile;$k>19;$k--) unset($xmldata->page[0]);
		}
	} else {
		$xmldata = @new SimpleXMLExtended('<?xml version="1.0" encoding="UTF-8"?><pages></pages>');
	}
	$page = $xmldata->addChild('page');
	$page->addChild('url',$url);
	$page->addChild('title',$title);
	$page->addChild('lastUpdate', date('Y-m-d H:i:s'));
	XMLsave($xmldata,LUPDATE_FILE);
// --- koniec wstawki
}
