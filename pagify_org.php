...
// start of modifications by wDEsign
    $pagingHtml .= '<div class="clearfix text-center"><nav><ul class="pagination">';
    if ($pageNum > 0) {
      $pagingHtml .= ' <li class="first"><a href="'.($link1 ? $link1 : str_replace('%PAGE%',1,$link)).'" title="'.i18n_r('pagify/TITLE_FIRST').'">'.i18n_r('pagify/FIRST').'</a></li>';
      $pagingHtml .= ' <li class="previous"><a href="'.($link1 && $pageNum == 1 ? $link1 : str_replace('%PAGE%',$pageNum,$link)).'" title="'.i18n_r('pagify/TITLE_PREVIOUS').'">'.i18n_r('pagify/PREVIOUS').'</a></li>';
    } else {
      $pagingHtml .= '<li class="first inactive"><span>'.i18n_r('pagify/FIRST').'</span></li>';
      $pagingHtml .= ' <li class="previous inactive"><span>'.i18n_r('pagify/PREVIOUS').'</span></li>';
    }
    ...
    if ($min > 0) $pagingHtml .= ' <li class="ellipsis"><span>'.i18n_r('pagify/ELLIPSIS').'</span></li>';
    for ($i=$min; $i<=$max; $i++) {
      if ($i == $pageNum) {
        $pagingHtml .= ' <li class="active current"><span>'.($i+1).'</span></li>';
      } else {
        $t = i18n_r('pagify/TO_PAGE');
        $pagingHtml .= ' <li><a href="'.($link1 && $i == 0 ? $link1 : str_replace('%PAGE%',$i+1,$link)).'" title="'.(trim($t) ? $t.($i+1) : '').'">'.($i+1).'</a></li>';
      }
    }
    if ($max < $numPages-1) $pagingHtml .= ' <li class="ellipsis"><span>'.i18n_r('pagify/ELLIPSIS').'</span></li>';
    if ($pageNum < $numPages-1) {
      $pagingHtml .= ' <li class="next"><a href="'.str_replace('%PAGE%',$pageNum+2,$link).'" title="'.i18n_r('pagify/TITLE_NEXT').'">'.i18n_r('pagify/NEXT').'</a></li>';
      $pagingHtml .= ' <li class="last"><a href="'.str_replace('%PAGE%',$numPages,$link).'" title="'.i18n_r('pagify/TITLE_LAST').'">'.i18n_r('pagify/LAST').'</a></li>';
    } else {
      $pagingHtml .= ' <li class="next inactive"><span>'.i18n_r('pagify/NEXT').'</span></li>';
      $pagingHtml .= ' <li class="last inactive"><span>'.i18n_r('pagify/LAST').'</span></li>';
    }
